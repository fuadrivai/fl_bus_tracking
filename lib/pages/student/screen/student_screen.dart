  ...
  saveToDb(BuildContext context) async {
    ...
    if (context.mounted) {
      Common.modalInfo(
        ...
        icon: const Icon(FontAwesomeIcons.circleCheck, color: Colors.green),
      );
      studentCheck = [];
      isLoading = false;
    }
        ...
        studentsView =
            studentsApi
                .where(
                  (data1) =>
                      !studentsDb.any(
                        (data2) => data1.childID == data2.childID,
                      ),
                )
                .toList();
        break;
...
class ListStudentWidget extends StatelessWidget {
  ...
  Widget checkboxListTile(Student student, Color color) {
    return CheckboxListTile(
      ...
      subtitle: Text(student.childDivision ?? "-"),
      secondary: Icon(FontAwesomeIcons.userAstronaut, size: 35, color: color),
    );
  }
}
